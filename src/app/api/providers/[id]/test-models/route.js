 ...
 * Body (optional): { models: [{ id: "model-id", kind: "llm" }] }
 * If models not provided, falls back to static config + /models API.
 */
export async function POST(request, { params }) {
  try {
    ...
    // Try to get models from request body first
    let models = [];
    try {
      const body = await request.json();
      if (Array.isArray(body?.models) && body.models.length > 0) {
        models = body.models.map((m) => ({ id: m.id || m, name: m.name || m.id || m, kind: m.kind || m.type || "llm" }));
      }
    } catch { /* no body or invalid JSON — continue to fallback */ }

    const baseUrl = `http://127.0.0.1:${process.env.PORT || UPDATER_CONFIG.appPort}`;

    // Fallback: static config + /models API
    if (models.length === 0) {
      models = getProviderModels(alias);
    ...
    let firstResult;
    try {
      firstResult = await pingModelByKind(`${alias}/${first.id}`, firstKind, baseUrl);
    } catch (e) {
      firstResult = { ok: false, latencyMs: 0, error: String(e?.message || e).slice(0, 240), status: 0 };
    }
      ...
      const restResults = await Promise.all(
        rest.map(async (model) => {
          try {
            ...
            return { modelId: model.id, name: model.name || model.id, ok: false, latencyMs: 0, error: String(e?.message || e).slice(0, 240), status: 0 };
          }
        })
      );
